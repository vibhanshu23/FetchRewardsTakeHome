...
//        let response = (try! JSONSerialization.jsonObject(with: details, options: [])) as! MealDetailsObjectFromServer
//        return response.getInterfaceObject()
    }

    let dummyURL = URL(string: "https://www.nyonyacooking.com/recipes/apam-balik~SJ5WuvsDf9WQ")

    func getDefaultNetworkHandler(url: URLType) -> NetworkHandler{
        switch url{
            case .meals:
